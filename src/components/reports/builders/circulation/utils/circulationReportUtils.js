...
export const mapFilterArray = (filterArray, fieldLabels) => {
  ...
  return filterArray.map((filter) => ({
    ...
    member: filter.member
      ...
      : filter.member,
    // Handle date range values from URL
...
export const generateQueryFromFormState = (formState) => {
  ...
  const allDimensions = [
    ...extractValues(formState.loanFields),
    ...extractValues(formState.patronFields),
  ...
  const filters = [
    ...processFilters(formState.loanFilters),
    ...processFilters(formState.patronFilters),
...
export const getInitialValuesFromURL = (fieldLabels) => {
  ...
  if (formStateFromURL) {
    try {
      ...
      return {
        ...parsed,
        loanFields: mapFieldArray(parsed.loanFields, fieldLabels),
        patronFields: mapFieldArray(parsed.patronFields, fieldLabels),
        itemFields: mapFieldArray(parsed.itemFields, fieldLabels),
        measures: mapFieldArray(parsed.measures, fieldLabels),
        loanFilters: mapFilterArray(parsed.loanFilters, fieldLabels),
        patronFilters: mapFilterArray(parsed.patronFilters, fieldLabels),
